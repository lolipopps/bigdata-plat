...
import com.bigdata.activity.utils.MessageUtil;
import com.bigdata.activity.vo.*;
...
@RestController
@Api(tags = "任务管理接口")
@RequestMapping("/bigdata/actTask")
@Transactional
public class ActTaskController {
    ...
    public Result historicFlow(@ApiParam("流程实例id") @PathVariable String id) {
    ...
    public Result pass(@ApiParam("任务id") @RequestParam String id,
    ...
    public Result passAll(@ApiParam("任务ids") @RequestParam String[] ids,
                                  @ApiParam("意见评论") @RequestParam(required = false) String comment,
                                  ActMessageVo m) {
    ...
    public Result backToTask(@ApiParam("任务id") @RequestParam String id,
    ...
    public void judgeParallelGateway(String procDefId) {
        ...
        for (ActivityImpl activityImpl : dfe.getActivities()) {
            ...
            if ("parallelGateway".equals(type)) {
                throw new BigdataException("流程设计中包含平行网关，暂不支持驳回（仅支持驳回至发起人）");
            }
        }
    }
    ...
    public Result back(@ApiParam("任务id") @RequestParam String id,
                               @ApiParam("流程实例id") @RequestParam String procInstId,
                               @ApiParam("意见评论") @RequestParam(required = false) String comment,
                               ActMessageVo m) {
    ...
    public Result backAll(@ApiParam("流程实例ids") @RequestParam String[] procInstIds,
                                  @ApiParam("意见评论") @RequestParam(required = false) String comment,
                                  ActMessageVo m) {
        ...
        for (String procInstId : procInstIds) {
            ...
            try {
                ...
                throw new BigdataException("请确保无重复所属的流程，或尝试对单条数据进行驳回");
            }
            ActBusiness actBusiness = actBusinessService.get(pi.getBusinessKey());
            actBusiness.setStatus(ActivitiConstant.STATUS_FINISH);
            actBusiness.setResult(ActivitiConstant.RESULT_FAIL);
    ...
    public Result delegate(@ApiParam("任务id") @RequestParam String id,
    ...
    public Result deleteHistoric(@ApiParam("任务id") @RequestParam String[] ids) {
