...
import com.bigdata.activity.service.*;
...
public class ActProcessController {
    ...
    public Result getByCondition(@ApiParam("是否显示所有版本") @RequestParam(required = false) Boolean showLatest,
    ...
    public Result updateStatus(@ApiParam("流程定义id") @RequestParam String id,
    ...
    public void exportResource(@ApiParam("流程定义id") @RequestParam String id,
        ...
        if (ActivitiConstant.RESOURCE_TYPE_XML.equals(type)) {
            ...
            throw new BigdataException("类型type不正确");
        }
        ...
        try {
            ...
            throw new BigdataException("导出部署流程资源失败");
        }
    }

    @RequestMapping(value = "/convertToModel/{id}", method = RequestMethod.GET)
    @ApiOperation(value = "转化流程为模型")
    public Result convertToModel(@ApiParam("流程定义id") @PathVariable String id) {
    ...
    public Result editStartUser(@ApiParam("流程定义id") @RequestParam String nodeId,
