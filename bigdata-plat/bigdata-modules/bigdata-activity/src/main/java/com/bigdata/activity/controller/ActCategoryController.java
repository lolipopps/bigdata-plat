...
@RestController
@Api(tags = "流程分类管理接口")
@RequestMapping("/bigdata/actCategory")
...
public class ActCategoryController {
    ...
    public Result add(ActCategory actCategory) {
    ...
    public Result edit(ActCategory actCategory) {
        ...
        // 若修改了分类名称
        if (!oldTitle.equals(actCategory.getTitle())) {
    ...
    public void deleteRecursion(String id, String[] ids) {
        ...
        if (list != null && list.size() > 0) {
            throw new BigdataException("删除失败，包含正被流程使用关联的分类");
        }
        // 获得其父节点
        ActCategory cat = actCategoryService.get(id);
        ActCategory parent = null;
        if (StrUtil.isNotBlank(cat.getParentId())) {
            parent = actCategoryService.findById(cat.getParentId());
        }
