...
import com.bigdata.activity.controller.modeler.DefaultProcessDiagramGenerator;
...
import com.bigdata.activity.vo.ActPage;
import com.bigdata.activity.vo.HistoricProcessInsVo;
import com.bigdata.activity.vo.ProcessInsVo;
import com.bigdata.activity.vo.ProcessNodeVo;
...
@RestController
@Api(tags = "流程实例管理接口")
@RequestMapping("/bigdata/actProcess")
@Transactional
public class ActProcessInsController {
    ...
    public Result getRunningProcess(@RequestParam(required = false) String name,
        ...
        list.forEach(e -> {
            ...
            // 关联流程表单路由
            ActProcess actProcess = actProcessService.findById(e.getProcDefId());
            if (actProcess != null) {
                e.setRouteName(actProcess.getRouteName());
            }
            // 关联业务表id
            ActBusiness actBusiness = actBusinessService.findById(e.getBusinessKey());
            if (actBusiness != null) {
    ...
    public Result getFinishedProcess(@RequestParam(required = false) String name,
                                             @RequestParam(required = false) String categoryId,
                                             @RequestParam(required = false) String key,
                                             SearchVo searchVo,
                                             PageVo pageVo) {
    ...
    public Result<ProcessNodeVo> getNode(@ApiParam("节点nodeId") @PathVariable String nodeId,
                                         ...
                                         @ApiParam("历史任务taskId") @RequestParam(required = false) String hisTaskId) {

        ProcessNodeVo node = actProcessService.getNode(nodeId, taskId, hisTaskId);
        return new ResultUtil<ProcessNodeVo>().setData(node);
    }
    ...
    public void getHighlightImg(@ApiParam("流程实例id") @PathVariable String id,
        ...
        try {
            ...
            throw new BigdataException("读取流程图片失败");
        }
    }

    @RequestMapping(value = "/updateInsStatus", method = RequestMethod.POST)
    @ApiOperation(value = "激活或挂起流程实例")
    public Result updateStatus(@ApiParam("流程实例id") @RequestParam String id,
    ...
    public Result delInsByIds(@RequestParam String[] ids,
    ...
    public Result delHistoricInsByIds(@RequestParam String[] ids) {
