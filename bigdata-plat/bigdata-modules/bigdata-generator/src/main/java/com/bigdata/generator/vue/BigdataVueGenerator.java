package com.bigdata.generator.vue;
...
import com.bigdata.generator.BigdataGenerator;
import com.bigdata.generator.bean.Field;
...
@RestController
@Api(tags = "Vue代码生成")
@RequestMapping(value = "/bigdata/generate")
public class BigdataVueGenerator {
    ...
    public String generate(String template, boolean api, boolean isDrawer, boolean enableTable, String vueName, Integer rowNum, List<Field> fields) throws IOException {
        ...
        tableTemplate.binding("vueName", BigdataGenerator.name(vueName, false));
        tableTemplate.binding("apiName", BigdataGenerator.name(vueName, true));
        ...
        if ("tree.btl".equals(template)) {
            if (!containsField(fields, "bigdataTreeTitle")) {
                fields.add(0, new Field().setType("bigdataTreeTitle").setSortOrder(new BigDecimal("-1")).setEditable(true));
            }
            if (!containsField(fields, "bigdataTreeChoose")) {
                fields.add(0, new Field().setType("bigdataTreeChoose").setSortOrder(new BigDecimal("-2")).setEditable(true));
            }
            if (!containsField(fields, "bigdataTreeSortOrder")) {
                fields.add(new Field().setType("bigdataTreeSortOrder").setSortOrder(new BigDecimal("999")).setEditable(true));
            }
        }
        ...
        if (rowNum == 1) {
            ...
            throw new BigdataException("rowNum仅支持数字1-4");
        }
        // 生成代码
